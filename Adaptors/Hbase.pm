...
sub storeMetaStart {
	my ( $self, $meta ) = @_;
	$self->storeMeta(  "Storing Sample=".$self->sample->name ." Analysis=" . $self->analysis->logic_name ." starting at " . localtime );
	return;
}

sub storeMetaFinish {
	my ( $self, $meta ) = @_;
	$self->storeMeta(  "Storing Sample=".$self->sample->name ." Analysis=" . $self->analysis->logic_name ." finished at " . localtime );
	return;
}
...
sub rangeQuery {
	...
	if ( $key ){
		...
		print STDERR "Destroying hash $key at position $chr : $position\nHASH : "
...
sub storeLookup {
	...
	# store meta data
	my $key = $self->analysis->dbID . ":" . $self->sample->dbID . ":LOOKUP:$batch";
	#print "$key\t$lookup\n";
  ...
  RESULT: while ( $result && @{$result} > 0 ) {
		...
		if ( $self->versions > 1 ) {
		  ...
		  #print STDERR 'Fetching ' . $self->versions ." versions. using $rk\n";
			$result = $self->tableOperation('getVer',$rk, "D", $self->versions );
		...
		foreach $var (@$result) {
			...
			my ( $cs,$ass,$chr,$start,$stop,$analysis,$sample)  = split( ":", $rk );
			...
			unless ( $last_chr eq $chr && $last_row eq $start){
				...
				if ($keys){
					foreach my $rk ( @$keys){
						...
						my $data = $self->rangeQuery($rangeAnalysis,$rangeSample,$chr,$start); 
...
#	print STDERR "Running client->$op('".$self->namespace."','".$value1,$value2,$value3."')\n";
	...
	if ($@) {
		$self->throw("Error running $op on table " . $self->namespace. " with value $value1,$value2,$value3\n$@\n");
	}
	return $result;
}
...
sub liftoverData {
	...
	print STDERR "CST " . $cst->name ." " . $cst->version ."\n";
	my $asm_mapper = $asma->fetch_by_CoordSystems( $csf, $cst );
	# variants rather than ranges
	my $end = $hash->{'END'};
	$end = $hash->{'START'} unless $end;
	my  @chr_coords = $asm_mapper->map( $hash->{'CHR'},$hash->{'START'},$end,1,  $csf );
	foreach my $c (@chr_coords){
		print STDERR "WHAT THE WHAT??$c\n";
		if ($c->isa("Bio::EnsEMBL::Mapper::Gap")){
			print STDERR  "GAP    " . $c->start ." " .$c->end ."\n";
		}
		if ($c->isa("Bio::EnsEMBL::Mapper::Coordinate")){
			my $slice = $sa->fetch_by_dbID($c->id);
			print STDERR  "COORDS " . $c->id ." " . $c->start ." " .$c->end ."\n";
		}
...
sub AssemblyMapperAdaptor {
	...
	# go fetch one
	my $am = $self->dbc->registry->get_adaptor($self->dbc->species,'core','assemblymapper');
...
sub CoordSystemAdaptor {
	...
	# go fetch one
	my $am = $self->dbc->registry->get_adaptor($self->dbc->species,'core','coordsystem');
...
sub SliceAdaptor {
	...
	# go fetch one
	my $am = $self->dbc->registry->get_adaptor($self->dbc->species,'core','slice');
