...
# connect to db
my ($client,$namespace,$mutator);
eval {
print "Connecting to nosql\t";
 $client = new Hypertable::ThriftClient("172.16.16.37", 38080);
print "Connected - getting name space\t";
 $namespace = $client->namespace_open("vcfTest");
print " got it opening mutator ";
 $mutator = $client->mutator_open($namespace, "vcf");
print " done\n";
}; if ($@) {
	print STDERR "Problem connecting to db :\n";
	print Dumper $@;
	exit;
}
my %Printable = ( "\\"=>'\\', "\r"=>'r', "\n"=>'n', "\t"=>'t', "\""=>'"' );
...
sub main {
	...
	# get analysis id
	my $aa = $config->{reg}->get_adaptor($config->{species}, "core", "analysis");
    my $analysis = $aa->fetch_by_logic_name('NOSQLTEST');
	...
	while(<$in_file_handle>) {
		...
		# data
			...
			$data->{info} = \%info;
			# make the keys and the values;
			my $rowkey = $seq_names{$data->{'#CHROM'}} .
				 $data->{'POS'}. ":" .
				 	 $data->{'POS'}. ":1";
			...
			foreach my $key (keys %$data ) {
				next if $key eq 'info' or $key eq 'line' or $key eq "#CHROM" or $key eq 'POS';
	
				if ($key eq $sample){
					# store them together with format
					htableStore($rowkey . "_nosqltest_" . $sample,'SAMPLE',$data->{$key});
					next;
				} 
				
				my $v = $data->{$key} ;
				# dont bother storing empty values
				next unless $v;
				next if $v eq ".";
				htableStore($rowkey . "_nosqltest_" . $sample,$key,$v);
			}

			#$batch =~ s/,$//;
			#my $cmd = "$batch\n";
		#	my $cmd = "echo \"USE 'script_PopulateDB'; INSERT INTO VCFcontents VALUES $batch; \" | /stornext/snfs0/next-gen/Illumina_test/delete/hypertable/hypertable-0.9.7.17-linux-x86_64/opt/hypertable/current/bin/ht shell --batch";
...
#			}
	...
	# store the header
	eval {
	my $key = new Hypertable::ThriftGen::Key({row => $sample . "_meta",column_family => 'header'});
	my $cell = new Hypertable::ThriftGen::Cell({key => $key,  value => $head});
	$client->mutator_set_cell($mutator, $cell);
	$client->mutator_flush($mutator);
    $client->mutator_close($mutator);
	}; if ($@){
		print STDERR "Problem finalizing connection\n";	
		print Dumper $@;
		exit;
	}
...
sub htableStore {
	my ($row,$column,$val) = @_;
	eval {
		my $hkey = new Hypertable::ThriftGen::Key({row => $row , column_family => $column});
		my $hcell = new Hypertable::ThriftGen::Cell({key => $hkey ,value => $val});
		#print "Attempting to store $row ,$column, $val \n";
		$client->mutator_set_cell($mutator, $hcell);
	}; if ($@){
		print STDERR "Error storing to hypertable:\n";
		print Dumper $@;
		exit;
	}
}
