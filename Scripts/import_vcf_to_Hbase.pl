...
=head1 NAME
import_vcf_to_NOSQL.pl - imports variations from a VCF file into an Ensembl variation DB and a NOSQL database
modified by Simon White for intergration with NOSQL (simonw@bcm.edu)
...
socketpair( CHILD, PARENT, AF_UNIX, SOCK_STREAM, PF_UNSPEC )
  or die "ERROR: Failed to open socketpair: $!";
CHILD->autoflush(1);
PARENT->autoflush(1);
...
if ( defined( $config->{fork} ) ) {
	...
	die "ERROR: Tabix index file "
	  . $config->{input_file}
	  . ".tbi not found, cannot fork\n"
...
# than potentially do the wrong thing
	  ...
	  or die
"ERROR: Failed to parse command line arguments - check the documentation!\n";
	...
	die("ERROR: Cannot run in test mode using forks\n")
	  if defined( $config->{fork} )
	  and defined( $config->{test} );
	die("ERROR: Cannot manually recover using forks\n")
	  if defined( $config->{fork} )
	  and (    defined( $config->{recover_point} )
			|| defined( $config->{recover_pos} ) );

	# recover?
	if ( defined $config->{recover} ) {
		open IN,
		  ( join '/', ( $ENV{HOME}, '.import_vcf', $config->{session_id} ) )
		  or die "ERROR: Could not recover session with ID "
		  . $config->{session_id} . "\n";
		my $a = <IN>;
		( $config->{recover_point}, $config->{pid} ) = split ' ', $a;
		close IN;
		die "ERROR: Cannot recover - session with ID "
		  . $config->{session_id}
		  . " has been flagged as finished"
		  if $config->{recover_point} eq 'FINISHED';
		debug(
			   $config,                  "Found recover point ",
			   $config->{recover_point}, " for session ID ",
	...
	if ( defined( $config->{remap} ) ) {
		die
"ERROR: remap argument incorrect - must be of format --remap [from_assembly],[to_assembly]"
		  unless $config->{remap} =~ /^\S+?\,\S+?$/;
	...
	$tables->{individual_genotype_multiple_bp} = 1
	  if defined( $config->{mart_genotypes} );
		 ...
		 || $tables->{compressed_genotype_var} )
	{
		$tables->{$_} = 1
		  for
		  qw/population individual individual_population sample variation variation_synonym variation_feature flanking_sequence/;
	}
	if (    $tables->{population_genotype}
		 || $tables->{compressed_genotype_region}
		 || $tables->{compressed_genotype_var} )
	{
		$tables->{$_} = 1
		  for
		  qw/genotype_code variation variation_synonym variation_feature flanking_sequence/;
	}
	if ( $tables->{allele} || $tables->{genotype_code} ) {
		$tables->{$_} = 1
		  for
		  qw/allele_code variation variation_synonym variation_feature flanking_sequence/;
	}
	...
	if ( defined $config->{only_existing} ) {
		$tables->{$_} = 0
		  for
		  qw/source variation variation_synonym variation_feature flanking_sequence/;
	}
	...
	if ( defined( $config->{cache} ) ) {
		die( "ERROR: Could not find cache directory " . $config->{cache} )
		  unless -e $config->{cache};
		$config->{vep}->{dir}     = $config->{cache};
		$config->{vep}->{cache}   = 1;
		$config->{vep}->{offline} = 1;
	}
	if ( defined( $config->{fasta} ) ) {
		die "ERROR: Specified FASTA file/directory not found"
		  unless -e $config->{fasta};
	...
	# connect to hbase
	# connect to db
	my $dbc = Adaptors::Connection->new(
										 -host     => "hadoop-headnode1",
										 -port     => "9090",
										 -registry => $config->{registry},
	);
	my $dba = Adaptors::VariantTable->new(
										   -con    => $dbc,
										   -buffer => 50000,
										   -table  => $config->{table}
	);
...
sub read_config_from_file {
	...
	close CONFIG;
	debug( $config, "Read configuration from $file" )
...
sub main {
	...
	# SET UP VARIABLES
	##################
	my ( %headers, $prev_seq_region, $genotypes, $var_counter, @vf_buffer, );
	...
	if ( defined( $config->{recover} ) ) {
		debug( $config,
			   "Importing data from recovered session's temporary files" );
		foreach
		  my $table (qw(allele population_genotype compressed_genotype_region))
		{
	...
	# backup
	backup($config)
	  if defined( $config->{backup} ) || defined( $config->{move} );

	# TEST MODE?
	############
	debug( $config, "Running in test mode - will read first ",
		   $config->{test}, " lines of input" )
	  if defined( $config->{test} );
	...
	$config->{slice_adaptor} =
	  $config->{reg}->get_adaptor( $config->{species}, "core", "slice" );
	my $sa = $config->{slice_adaptor};

	# cache which coord system the seq regions are on
	my %cs;
	my $ass;
	foreach my $slice ( @{ $sa->fetch_all('toplevel') } ) {
		...
		my @tmp = split( ":", $name );
		$ass = $tmp[1];
		$cs{ $tmp[2] } = substr( $tmp[0], 0, 1 );
	}

	# give assembly to hbase adaptor
	$config->{hbase}->assembly($ass);

	# give analysis to hbase adaptor
	$config->{hbase}->analysis( $config->{analysis} );

	# get/set source_id
	die("ERROR: no source specified\n")
	  if !( defined $config->{source} ) && !defined( $config->{only_existing} );
	$config->{source_id} = get_source_id($config)
	...
	if ( defined $config->{disable_keys} ) {
		...
		{    #grep {$config->{tables}->{$_}} keys %$config->{tables}) {
	...
	while (<$in_file_handle>) {
		...
		if (/^#/) {
			...
			# write the meta data to say we have started importing
			$config->{hbase}->storeMetaStart();
		}
		...
		# data
			...
			# recover?
			if (    defined( $config->{recover_point} )
				 || defined( $config->{recover_pos} ) )
			{
				$config->{skipped}->{already_processed}++;
				if ( defined( $config->{recover_point} )
					 && md5_hex( $data->{line} ) eq $config->{recover_point} )
				{
			...
			if (
				 !defined(             $config->{seq_region_ids}->{ $data->{tmp_vf}->{chr} }
				 )
			  )
			{
			...
			# copy seq region ID
			$data->{tmp_vf}->{seq_region_id} =
			  $config->{seq_region_ids}->{ $data->{tmp_vf}->{chr} };
...
#			}
			...
			$var_counter++;
			last
			  if defined( $config->{test} ) && $var_counter == $config->{test};
			if ( defined( $config->{forked} ) ) {
				debug(
					   $config,
					   "Processed $var_counter lines ("
						 . $config->{forked} . ")"
				);
				store_session( $config, md5_hex( $data->{line} ) )
				  if $var_counter % $config->{progress_update} == 0;
			} elsif ( $var_counter % $config->{progress_update} == 0 ) {
				progress( $config, $var_counter );
				store_session( $config, md5_hex( $data->{line} ) )
				  unless defined( $config->{no_recover} );
			}
		}
	}
...
# re-enable keys if requested
		 ...
		 || defined( $config->{recover_pos} ) )
	{
		if ( defined( $config->{forked} ) ) {
			debug(
				   $config,
				   "WARNING: Could not find recovery point "
					 . (
						 defined( $config->{recover_point} )
						 ? $config->{recover_point}
						 : $config->{recover_pos}
					 )
					 . " for session "
					 . $config->{session_id}
	...
	my $max_length = (
			 sort  { $a <=> $b }
			   map { length($_) }
			   ( keys %{ $config->{skipped} }, keys %{ $config->{rows_added} } )
	)[-1];

	# rows added
	debug( $config,
		   ( defined( $config->{test} ) ? "(TEST) " : "" ) . "Rows added:" );
	for my $key ( sort keys %{ $config->{rows_added} } ) {
		debug(
			   $config,
			   ( defined( $config->{forked} ) ? "STATS\t" : "" ) . $key
				 . ( ' ' x ( ( $max_length - length($key) ) + 4 ) )
				 . $config->{rows_added}->{$key}
		);
	}
	...
	for my $key ( sort keys %{ $config->{skipped} } ) {
		debug(
			   $config,
			   ( defined( $config->{forked} ) ? "SKIPPED\t" : "" ) . $key
				 . ( ' ' x ( ( $max_length - length($key) ) + 4 ) )
				 . $config->{skipped}->{$key}
		);
	}
...
"$remote_test\nERROR: Could not find file or index file for remote annotation file $filepath\n";
			...
			debug(
				$config,
"Downloaded tabix index file for remote annotation file $filepath"
			  )
			  unless defined( $config->{quiet} );
		}
	}
	...
	if ( scalar @chrs < $config->{fork} ) {
		debug( $config,
			   "Found only " . ( scalar @chrs ) . " chromosomes, subdividing" );
		my ( %min_pos, %max_pos );
		...
		while (<TMP>) {
			...
			$max_pos{$chr} = $pos
			  if !defined( $max_pos{$chr} )
			  or $pos > $max_pos{$chr};
			$min_pos{$chr} = $pos
			  if !defined( $min_pos{$chr} )
			  or $pos < $min_pos{$chr};
			$num_lines++;
		}
		close TMP;
		my $size = (
					 scalar @chrs == 1
					 ? int(
						   ( $max_pos{ $chrs[0] } - $min_pos{ $chrs[0] } + 1 ) /
							 $config->{fork}
					   )
					 : 10000000
		);
...
#my @chrs = sort {$chr_hash{$a} <=> $chr_hash{$b}} keys %chr_hash;#(1..22,'X','Y','MT');
	...
	debug(
		   $config,
		   "Done - found $string variants across "
			 . ( scalar @chrs )
			 . " chromosomal regions"
	);
	...
	if ( $comm_pid == 0 ) {
		...
		while (<CHILD>) {
			...
			if (/Processed/) {
				...
				progress( $config, $c, scalar keys %in_progress )
				  if $c % $config->{progress_update} == 0;
			...
			*STDERR = *PARENT;
			...
			if ( defined $config->{recover} ) {
				if (
					 ...
					 (
						join '/',
						( $ENV{HOME}, '.import_vcf', $config->{session_id} )
					 )
				  )
				{
					my $a = <IN>;
					( $config->{recover_point}, $config->{pid} ) = split ' ',
					  $a;
					close IN;
 ...
 # delete the old recovery point since this will be the one for the main session
	...
	unlink(
			join '/',
			(
			   $ENV{HOME}, '.import_vcf', md5_hex( $config->{session_id} . $_ )
			)
	  )
	  for @chrs;
	...
	if ( defined( $config->{database} ) ) {
		...
		if ( defined( $config->{host} ) && defined( $config->{user} ) ) {
			...
			if ( -e $config->{registry} ) {
				...
				die "ERROR: could not read from registry file "
				  . $config->{registry} . "\n";
			}
		}
		...
		my $vdba = $reg->get_DBAdaptor( $config->{species}, 'variation' )
		  || usage(   "Cannot find variation db for "
					. $config->{species} . " in "
					. $config->{registry_file} );
		$config->{dbVar} = $vdba->dbc->db_handle;
		debug( $config, "Connected to database ",
			   $vdba->dbc->dbname, " on ", $vdba->dbc->host, " as user ",
			   $vdba->dbc->username );
	}
}
...
sub get_adaptors {
	...
	{
		$config->{ $type . '_adaptor' } =
		  $config->{reg}->get_adaptor( $config->{species}, "variation", $type );
...
# special case, population genotype adaptor needs a pointer to allele adaptor for caching allele codes
	$config->{populationgenotype_adaptor}->{_allele_adaptor} =
	  $config->{allele_adaptor};
	...
	if (
		 (
		   defined( $config->{tables}->{transcript_variation} )
		   && $config->{tables}->{transcript_variation}
		 )
		 || defined( $config->{remap} )
	  )
	{
		$config->{slice_adaptor} =
		  $config->{reg}->get_adaptor( $config->{species}, "core", "slice" );
...
sub get_input_file_handle {
	...
	if ( defined( $config->{input_file} ) ) {
		...
		if ( $config->{input_file} =~ /\.gz$/ ) {
			$in_file_handle->open(
								 "zcat " . $config->{input_file} . "  2>&1 | " )
			  or die( "ERROR: Could not read from input file ",
					  $config->{input_file}, "\n" );
		} elsif ( $config->{input_file} =~ /\.vcf$/ ) {
			$in_file_handle->open( $config->{input_file} )
			  or die( "ERROR: Could not read from input file ",
					  $config->{input_file}, "\n" );
		} else {
			die "ERROR: Not sure how to handle file type of ",
			  $config->{input_file}, "\n";
		}
...
sub sql_populate {
	...
	my $sql;
	open SQL, $sql_file
	  or die "ERROR: Could not read from SQL file $sql_file\n";
	my $comment = 0;
	while (<SQL>) {
		chomp;
		s/^\s+//g;
	...
	if ( defined( $config->{test} ) ) {
		...
		foreach my $command ( split ';', $sql ) {
			eval {
				$config->{dbVar}->do($command)
				  or warn $config->{dbVar}->errstr;
			};
		}
	}
}
...
sub backup {
	...
	if ( defined( $config->{move} ) ) {
		foreach my $table (
							grep { $config->{tables}->{$_} }
							keys %{ $config->{tables} }
		  )
		{
			debug(
				   $config,
				   ( defined( $config->{test} ) ? "(TEST) " : "" )
					 . "Renaming $table to $table\_$pid"
			);
			if ( !defined( $config->{test} ) ) {
		  ...
		  )
		{
			debug(
				   $config,
				   ( defined( $config->{test} ) ? "(TEST) " : "" )
					 . "Backing up table $table as $table\_$pid"
			);
			if ( !defined( $config->{test} ) ) {
				$config->{dbVar}->do(qq{CREATE TABLE $table\_$pid LIKE $table});
				$config->{dbVar}
				  ->do(qq{INSERT INTO $table\_$pid SELECT * FROM $table});
			}
		}
	}
}
...
sub attrib {
	...
	if ( !-e $script ) {
		debug( $config,
			   "WARNING: Could not find create_attrib_sql.pl in ../misc/" );
		return;
	}
	...
	my $dbh = DBI->connect( 'DBI:mysql:;host=ensembldb.ensembl.org;port=5306',
							'anonymous' );
	$sth = $dbh->prepare(
		qq{
		SHOW DATABASES LIKE ?
	...
	if ( !defined($db_name) ) {
		$sth->execute(
				'homo_sapiens_variation_' . ( $installed_version - 1 ) . '_%' );
		$sth->bind_columns( \$db_name );
		$sth->fetch;
		$sth->finish;
		if ( !defined($db_name) ) {
			debug( $config,
"WARNING: Could not find database of version $installed_version or "
				  . ( $installed_version - 1 )
				  . " on ensembldb.ensembl.org; can't populate attrib tables" );
			return;
		}
	}
	...
	if (
		system(
			sprintf(
'perl %s --config Bio::EnsEMBL::Variation::Utils::Config --host ensembldb.ensembl.org --user anonymous --port 5306 --db %s > /tmp/attribs.sql',
				$script, $db_name
			)
		) == 0
	  )
	{
		...
		debug(
			$config,
"WARNING: Failed to populate attrib table. Variation classses will not be set correctly"
		);
	}
	return;
}
...
sub copy_seq_region_from_core {
	...
	my $cdba = $config->{reg}->get_DBAdaptor( $config->{species}, 'core' )
	  or return {};
...
sub store_session {
	...
	if ( open SESSION, "> $file" ) {
		...
		debug(
			$config,
"WARNING: Could not write to session recover file $file - you will not be able to recover this session"
		);
		$config->{no_recover} = 1;
	}
}
...
sub get_seq_region_ids {
	...
	if ( defined( $config->{test} ) ) {
		debug( $config, "Loaded ",
			   scalar keys %seq_region_ids,
			   " entries from seq_region table" );
	}
...
sub get_source_id {
	...
	if ( !defined($source_id) ) {
		if ( defined( $config->{test} ) ) {
			debug( $config,
				   "(TEST) Writing source name $source to source table" );
		} else {
			$sth =
			  $dbVar->prepare(
						 qq{insert into source(name, description) values(?,?)});
			$sth->execute( $source, $desc );
			$sth->finish();
			$source_id =
			  $dbVar->last_insert_id( undef, undef, qw(source source_id) );
		}
...
sub population {
	...
	if ( defined( $config->{panel} ) ) {
		open PANEL, $config->{panel}
		  or die "ERROR: Could not read from panel file "
		  . $config->{panel} . "\n";
		my $pop_inds = {};
		my $ind_pops = {};
		while (<PANEL>) {
			...
			push @split, $config->{population}
			  if defined( $config->{population} );
	...
	die
"ERROR: Population name not specified - use --population [population] or --panel [panel_file]\n"
	  unless scalar @pops;
	...
	if ( defined( $config->{gmaf} ) ) {
		die "ERROR: Population specified using --gmaf ("
		  . $config->{gmaf}
		  . " is not one of those to be added; use \"--gmaf ALL\" to calculate GMAF from all individuals in the file\n"
		  unless grep {
			     $config->{gmaf} eq 'ALL'
			  or $config->{gmaf} eq $_
			  or $config->{pop_prefix}
			  . $config->{gmaf} eq $_
		  } @pops;
	}
	my @return;
	foreach my $pop_name (@pops) {
		...
		if ( !defined($pop) ) {
			$pop = Bio::EnsEMBL::Variation::Population->new(          -name => $pop_name,
															 -adaptor => $pa, );
			if ( defined( $config->{test} ) ) {
				debug( $config,
					   "(TEST) Writing population object named $pop_name" );
			} else {
				$pa->store($pop);
			}
...
sub pedigree {
	...
	while (<IN>) {
		...
		$ped->{$ind}->{gender} =
		  defined($sex) ? ( $genders{$sex} || 'Unknown' ) : 'Unknown';
...
# populate %ind_pops hash if it doesn't exist (this will happen when using --population but no panel file)
	if ( !exists( $config->{ind_pops} ) ) {
		my %ind_pops = map {
			    $config->{ind_prefix}
			  . $_ => { $config->{pop_prefix} . $config->{population} => 1 }
		} @$split_ref;
		$config->{ind_pops} = \%ind_pops;
	}
...
# need the relationship to go both ways (allele/pop_genotype uses this way round later on)
	if ( !exists( $config->{pop_inds} ) ) {
		my %pop_inds;
		$pop_inds{ $config->{pop_prefix} . $config->{population} }
		  ->{ $config->{ind_prefix} . $_ } = 1
		  for @$split_ref;
		$config->{pop_inds} = \%pop_inds;
	}
...
# only add the individuals that were defined in the panel file (this will be all if no panel file)
	my @sorted =
	  grep { defined $config->{ind_pops}->{ $config->{ind_prefix} . $_ } }
	  @$split_ref;

# if we have pedigree, we need to sort it so the parent individuals get added first
	if ( defined( $config->{pedigree} )
		 && ref( $config->{pedigree} ) eq 'HASH' )
	{
		@sorted = sort {
			(
			   defined(
				   $config->{pedigree}->{ $config->{ind_prefix} . $a }->{father}
				 ) + defined(
				   $config->{pedigree}->{ $config->{ind_prefix} . $a }->{mother}
				 )
			  ) <=> (
					  defined(
							 $config->{pedigree}->{ $config->{ind_prefix} . $b }
							   ->{father}
						) + defined(
							 $config->{pedigree}->{ $config->{ind_prefix} . $b }
							   ->{mother}
	...
	foreach my $individual_name (@sorted) {
		...
		if ( scalar @$inds > 1 ) {
			die
"ERROR: Multiple individuals with name $individual_name found, cannot continue\n";
		} elsif ( scalar @$inds == 1 ) {
			$ind = $inds->[0];
		}
		...
		# create new
			...
			$ind->{populations} = [
							 map { $pop_objs{$_} }
							   keys %{ $config->{ind_pops}->{$individual_name} }
			];

			# add data from pedigree file
			if (    defined( $config->{pedigree} )
				 && ref( $config->{pedigree} ) eq 'HASH'
				 && ( my $ped = $config->{pedigree}->{$individual_name} ) )
			{
				$ind->{gender} = $ped->{gender} if defined( $ped->{gender} );
				$ind->{father_individual} = $ind_objs{ $ped->{father} }
				  if defined( $ped->{father} );
				$ind->{mother_individual} = $ind_objs{ $ped->{mother} }
				  if defined( $ped->{mother} );
			}
			if ( $config->{tables}->{individual} ) {
				if ( defined( $config->{test} ) ) {
					debug(
						$config,
"(TEST) Writing individual object named $individual_name"
					);
				} else {
					$ia->store($ind);
				}
				...
				$config->{rows_added}->{sample}++;
				$config->{rows_added}->{individual_population} +=
				  scalar @{ $ind->{populations} };
			}
		}
...
sub variation {
	...
	my $class_id =
	  $config->{attribute_adaptor}
	  ->attrib_id_for_type_value( 'SO_term', $so_term );
		...
		$var = Bio::EnsEMBL::Variation::Variation->new_fast(
							   {
								 ...
								 ancestral_allele => $data->{info}->{AA} eq '.'
								 ? undef
								 : uc( $data->{info}->{AA} )
							   }
		);
...
sub variation_feature {
	...
	# remove Ns?
	@new_alleles = grep { $_ ne 'N' } @new_alleles
	  if defined( $config->{skip_n} );
	...
	my $existing_vfs = $var_in_db
	  ...
	  : $vfa->_fetch_all_by_coords( $config->{seq_region_ids}->{ $vf->{chr} },
									$vf->{start}, $vf->{end},
									$config->{somatic} );
	...
	# check existing VFs
	foreach my $existing_vf (
		sort {
			( count_common_alleles( $vf->allele_string, $b->allele_string )
				 <=> count_common_alleles( $vf->allele_string,
										   $a->allele_string ) )
			  || ( $a->map_weight <=> $b->map_weight )
			  || ( $b->source eq 'dbSNP' ) <=> ( $a->source eq 'dbSNP' )
			  || ( split 'rs', $a->variation_name )[-1]
			  <=> ( split 'rs', $b->variation_name )[-1]
		} @$existing_vfs
	  )
	{
		...
		if ( scalar keys %combined_alleles > scalar @existing_alleles ) {
			...
			if ( defined( $config->{test} ) ) {
				debug(
					   $config,
					   "(TEST) Changing allele_string for ",
					   $existing_vf->variation_name,
					   " from ",
					   $existing_vf->allele_string,
					   " to $new_allele_string"
				);
			} else {
		...
		if (     $existing_vf->variation_name ne $data->{ID}
			 and !defined( $config->{only_existing} )
			 and !$added_synonym
			 and !defined( $data->{made_up_name} ) )
		{
			if ( defined( $config->{test} ) ) {
				debug(
					   $config,
					   "(TEST) Adding ",
					   $data->{ID},
					   " to variation_synonym as synonym for ",
					   $existing_vf->variation_name
				);
			} else {
				my $sth = $dbVar->prepare(
				...
				$sth->execute(
							   $existing_vf->{_variation_id}
								 || $existing_vf->variation->dbID,
							   $config->{source_id},
							   $data->{ID}
				);
				$sth->finish;
		...
		# add GMAF?
		if ( defined( $config->{gmaf} )
			 && !defined( $data->{variation}->minor_allele_frequency ) )
		{
	...
	if ( !defined( $vf->{dbID} ) && !defined( $config->{only_existing} ) ) {
		...
		if ( defined( $config->{test} ) ) {
			debug( $config,
				   "(TEST) Writing variation object named ",
				   $data->{variation}->name )
			  unless defined( $data->{variation}->dbID );
		} else {
...
#$config->{variation_adaptor}->store($data->{variation}) unless defined($data->{variation}->{dbID});
		...
		# now store the VF
		if ( defined( $config->{test} ) ) {
			debug( $config, "(TEST) Writing variation_feature object named ",
				   $vf->variation_name );
		} else {

			#$vfa->store($vf);
			#print STDERR "Me no storey\n";
			# this is where we call the nosql routine
			#print $vf->variation->minor_allele ."\n";
		}

		# update meta_coord stat
		$config->{meta_coord}->{variation_feature} =
		  $vf->{end} - $vf->{start} + 1
		  if !defined( $config->{meta_coord}->{variation_feature} )
		  or $vf->{end} - $vf->{start} + 1 >
		  $config->{meta_coord}->{variation_feature};
...
sub remap {
	...
	my $slice =
	  $config->{slice_adaptor}
	  ->fetch_by_region( 'chromosome', $vf->{chr}, undef, undef, undef,
						 $config->{remap}->[0] );
	...
	eval {
		@segments = @{
			$feat->feature_Slice->project(
										   'chromosome', $config->{remap}->[1]
			)
		  };
	};
	if ($@) {
	...
	@slices_newdb_newasm =
	  sort { $a->start <=> $b->start } @slices_newdb_newasm;
	...
	foreach my $vf (@$vfs) {
		...
		foreach my $tv ( @{ $vf->get_all_TranscriptVariations } ) {
			...
			if ( defined( $config->{test} ) ) {
				debug(
					$config,
					"(TEST) Writing transcript_variation object for variation ",
					$vf->variation_name,
					", transcript ",
...
sub get_genotypes {
	...
	for my $i ( 9 .. ( ( scalar @$split ) - 1 ) ) {
		...
		push @genotypes,
		  Bio::EnsEMBL::Variation::IndividualGenotype->new_fast(
							{
...
sub add_gmaf {
	...
	return
	  unless scalar( grep { length($_) == 1 } @alleles ) == scalar @alleles;
	...
	if ( defined( $data->{genotypes} ) ) {
		map { $counts{$_}++ }
		  ...
		  grep {
			     $config->{gmaf} eq 'ALL'
			  || $config->{pop_inds}->{ $config->{gmaf} }
			  ->{ $_->{individual}->{name} }
			  || $config->{pop_inds}
			  ->{ $config->{pop_prefix} . $config->{gmaf} }
			  ->{ $_->{individual}->{name} }
		  } @{ $data->{genotypes} };
		$total += $_ for values %counts;
		%freqs = map {
			$_ => ( defined( $counts{$_} ) ? ( $counts{$_} / $total ) : 0 )
		} @alleles;
		if ( %freqs && %counts ) {
			$var->{minor_allele} =
			  ( sort { $freqs{$a} <=> $freqs{$b} } keys %freqs )[0];
			$var->{minor_allele_frequency} =
			  ( sort { $a <=> $b } values %freqs )[0];
			$var->{minor_allele_count} =
			  ( sort { $a <=> $b } values %counts )[0];
		}
	}
}
	...
	foreach my $pop ( @{ $config->{populations} } ) {
		...
		if ( defined( $data->{genotypes} ) ) {
			map { $counts{$_}++ }
			  ...
			  map  { @{ $_->{genotype} } }
			  grep {
				$config->{pop_inds}->{$pop_name}->{ $_->{individual}->{name} }
			  } @{ $data->{genotypes} };
			$total += $_ for values %counts;
			@freqs =
			  map { defined( $counts{$_} ) ? ( $counts{$_} / $total ) : 0 }
			  @alleles;
		}
		for my $i ( 0 .. $#alleles ) {
			...
			if ( defined( $config->{test} ) ) {
				debug(
					   $config,
					   "(TEST) Writing allele object for variation ",
					   $data->{variation}->name,
					   ", allele ",
					   $alleles[$i],
					   ", population ",
	...
	if ( defined( $config->{recover_check} ) ) {
		...
		foreach my $a (@objs) {
			...
			foreach my $b (@$db_alleles) {
				if (
					$a->allele eq $b->allele
					and
					( defined $b->{_population_id} or defined $b->population )
					and $a->population->dbID eq (
												 defined( $b->{_population_id} )
												 ? $b->{_population_id}
												 : $b->population->dbID
					)
					and (
						(
						   ...
						   or ( !defined( $a->count ) && !defined( $b->count ) )
						)
						or (
							 (
							      defined( $a->frequency )
							   && defined( $b->frequency )
							   && substr( $a->frequency, 0, 4 ) ==
							   substr( $b->frequency, 0, 4 )
							 )
							 or (    !defined( $a->frequency )
								  && !defined( $b->frequency ) )
						)
					)
				  )
	...
	foreach my $pop ( @{ $config->{populations} } ) {
		...
		foreach my $gt_string ( keys %freqs ) {
			...
			if ( defined( $config->{test} ) ) {
				debug(
					 $config,
					 "(TEST) Writing population_genotype object for variation ",
					 $data->{variation}->name,
					 ", genotype ",
					 $gt_string,
					 ", population ",
	...
	if ( defined( $config->{recover_check} ) ) {
		...
		foreach my $a (@objs) {
			...
			foreach my $b (@$db_popgts) {
				if (
					$a->genotype_string eq $b->genotype_string
					and
					( defined $b->{_population_id} or defined $b->population )
					and $a->population->dbID eq (
												  defined $b->{_population_id}
...
sub individual_genotype {
	...
	if ( defined( $config->{test} ) ) {
		...
		debug( $config, "(TEST) Writing ",
			   scalar @gts,
			   " genotype objects for variation ",
			   $data->{variation}->name );
	} else {
		...
		if ( defined( $config->{recover_check} ) ) {
			...
			foreach my $a (@gts) {
				my $matched = 0;
				foreach my $b (@$db_gts) {
					if (     $a->genotype_string eq $b->genotype_string
						 and defined $b->individual
						 and $a->individual->dbID eq $b->individual->dbID )
					{
						$matched = 1;
						$count++;
						last;
			...
			# all objects novel, must be onto all new stuff
			delete $config->{recover_check}
			  if scalar @gts == scalar @final_objs;
			return unless scalar @final_objs;
			@gts = @final_objs;
		}
		...
		if ( $config->{mart_genotypes} ) {
			my $table =
			  $data->{vf}->{allele_string} =~ /^[ACGTN](\/[ACGTN])+$/
...
sub meta_coord {
	...
	# get coord system ID
	my $csa =
	  $config->{reg}->get_adaptor( $config->{species}, "core", "coordsystem" );
	my $cs = $csa->fetch_by_name( $config->{coord_system} );
	return unless defined $cs;
	my $qsth = $config->{dbVar}->prepare(
		q{
		SELECT max_length
		FROM meta_coord
		WHERE table_name = ?
	...
	foreach my $table ( keys %{ $config->{meta_coord} } ) {
		...
		if ( defined($existing_length) ) {
			...
			if ( defined( $config->{meta_coord}->{$table} )
				 and $existing_length < $config->{meta_coord}->{$table} )
			{
				if ( defined( $config->{test} ) ) {
					debug( $config,
						   "(TEST) Updating meta_coord entry for $table" );
				} else {
					$usth->execute( $config->{meta_coord}->{$table},
									$table, $cs->dbID );
				}
			}
		}
		...
		# row doesn't exist
			...
			if ( defined( $config->{test} ) ) {
				...
				$isth->execute( $table, $cs->dbID,
								$config->{meta_coord}->{$table} );
			}
		}
	}
	$qsth->finish;
...
sub get_tmp_file_handle {
	...
	if ( !defined( $config->{handles}->{$table} ) ) {
		my $file_handle =
		  FileHandle->new( "> " . get_tmp_file_name( $config, $table ) );
...
sub import_tmp_file {
	...
	# update meta_coord stat
	$config->{meta_coord}->{$table} = DISTANCE + 1
	  if $table eq 'compressed_genotype_region';
	if ( defined( $config->{test} ) ) {
		...
		if ( -e $file ) {
			my $call =
			  "mv $file " . $config->{tmpdir} . "/" . $config->{tmpfile};
			system($call);
			...
			my @columns =
			  $config->{ $adaptors{$table} . '_adaptor' }->_write_columns;
...
sub print_file {
	...
	# get file handle
	my $file_handle =
	  get_tmp_file_handle( $config, 'compressed_genotype_region' );
	if ( defined( $config->{test} ) ) {
		debug( $config,
			   "(TEST) Writing genotypes for ",
			   ( scalar keys %$genotypes ),
			   " individuals to temp file" );
	} else {
...
sub progress {
	...
	printf(
"\r%s - Processed %8i variants (%8.2f per min / %8.2f per min overall ) ( %2i process%2s )",
		getTime, $count, $prev_pm, $total_pm, $proc,
		( $proc > 1 ? 'es' : '  ' ) );
	$config->{prev_time} = [gettimeofday];
}
